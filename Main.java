...
public class Main
{
    // calculates all non-null values
    static private <T> int calculateNull(T[] collection) {
        int res = 0;
    ...
    static private <T> int calculateNotNull(T[] collection) {
        int res = 0;
        for (int i = 0; i < collection.length; i++){
            if(collection[i] != null){
                res += 1;
    ...
    static private <T extends Thread> boolean allThreadState(T[] collection, State threadState){
        boolean check = true;
        for (T t : collection) {
            if (t.getState() != threadState ){
                check = false;
            } 
        }
        return check;
    ...
    static private <T extends Thread> boolean checkThreadStateContains(T[] collection, State threadState){
        for (T t : collection) {
            if (t.getState() == threadState ){
                return true;
            } 
    ...
    public static void main(String[] args)
    {
        ...
        while (time < timerLength) 
        {
            ...
            for (Sack sack : sacks) {
                giftsInSacks += calculateNotNull(sack.accumulation);
            }

            int giftsInHoppers = 0;
            for (Hopper hopper : hoppers) {
                giftsInHoppers += calculateNotNull(hopper.collection);
            }
        ...
        // Stop the tables!   
        // While all tables are not all waiting or all runnable
        // or any of the tables are 'timed waiting' - e.g. sleeping as the program has told them to
        // keep running
        while((!allThreadState(tables, Thread.State.WAITING) && !allThreadState(tables, State.RUNNABLE)) || checkThreadStateContains(tables, State.TIMED_WAITING)){
        ...
        for (int h = 0; h < numHoppers; h++)
        {
            int hopperDeposited = 0;
            int filledCapacity = hoppers[h].collection.length - hoppers[h].getPresentsLoaded();
            hopperDeposited = calculateNull(hoppers[h].collection) - filledCapacity;
            giftsDeposited += hopperDeposited;
            System.out.println("Hopper " + hoppers[h].id + " deposited " + hopperDeposited + " presents and waited " + hoppers[h].timeWaiting / 1000 +"s.");
        }
        System.out.println();
        ...
        // Calculate gifts on belts
        for (Conveyor belt : belts) {
            giftsOnMachine += belt.countBelt();
        }
        // Calculate gifts on turn tables
